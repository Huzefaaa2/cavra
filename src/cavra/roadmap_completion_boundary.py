...
def validate_repository(repo_root: Path) -> dict[str, Any]:
    ...
    future_phase_registry_path = repo_root / "docs/roadmap-future-phase-registry.md"
    wiki_future_phase_registry_path = repo_root / "docs/wiki/Roadmap-Future-Phase-Registry.md"
    future_work_governance_index_path = repo_root / "docs/roadmap-future-work-governance-index.md"
    wiki_future_work_governance_index_path = repo_root / "docs/wiki/Roadmap-Future-Work-Governance-Index.md"
    governance_quickcheck_path = repo_root / "docs/roadmap-governance-quickcheck.md"
    wiki_governance_quickcheck_path = repo_root / "docs/wiki/Roadmap-Governance-Quickcheck.md"
    ...
    future_phase_registry_text = _read(future_phase_registry_path, blockers)
    wiki_future_phase_registry_text = _read(wiki_future_phase_registry_path, blockers)
    future_work_governance_index_text = _read(future_work_governance_index_path, blockers)
    wiki_future_work_governance_index_text = _read(wiki_future_work_governance_index_path, blockers)
    governance_quickcheck_text = _read(governance_quickcheck_path, blockers)
    wiki_governance_quickcheck_text = _read(wiki_governance_quickcheck_path, blockers)
    ...
    for path, text in (
        (future_phase_registry_path, future_phase_registry_text),
        (wiki_future_phase_registry_path, wiki_future_phase_registry_text),
    ):
        _require_fragments(
            path,
            text,
            [
                ...
                "This registry does not add `R7.62`",
                "The future phase registry is not itself a new product phase",
            ],
            blockers,
        )

    for path, text in (
        (future_work_governance_index_path, future_work_governance_index_text),
        (wiki_future_work_governance_index_path, wiki_future_work_governance_index_text),
    ):
        _require_fragments(
            path,
            text,
            [
                ...
                "This index does not add `R7.62`",
                "The future work governance index is not itself a new product phase",
            ],
            blockers,
        )

    for path, text in (
        (governance_quickcheck_path, governance_quickcheck_text),
        (wiki_governance_quickcheck_path, wiki_governance_quickcheck_text),
    ):
        _require_fragments(
            path,
            text,
            [
                ...
                "ready_to_operate_closed_roadmap_governance",
                "does not add a roadmap row",
                "Roadmap Future Work Governance Index",
            ],
            blockers,
        )
...
def _validate_roadmap(path: Path, text: str, blockers: list[str]) -> dict[str, Any]:
    ...
    over_boundary = [
        row.row_id
        for row in rows
        if row.row_id.startswith("R7.") and int(row.row_id.split(".", 1)[1]) > MAX_PHASE7_ROW
    ]
    if over_boundary:
