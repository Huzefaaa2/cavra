...
The public schema is maintained at
[`docs/release-packets/community-ga-release-packet.schema.json`](release-packets/community-ga-release-packet.schema.json).
A safe example packet is maintained at
[`examples/release-packets/community-ga-release-packet.example.json`](../examples/release-packets/community-ga-release-packet.example.json).
...
The JSON packet should mirror the Markdown packet and include:

- release identity: `packet_id`, `release_name`, `release_state`, `prepared_at`;
- repository references: `repository`, `branch`, `pull_request`, `commit`,
  `tag`, and `wiki_sync_commit`;
- gate results for every gate in the Community GA release checklist;
- validation commands and their public-safe results;
- accepted risks with owner, expiry, and compensating control;
- public boundary review flags;
- final decision and next recommended work.
