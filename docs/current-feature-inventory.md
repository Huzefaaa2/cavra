...
Approval router: `approval create`, `list`, `approve`, `deny`, `expire`, `break-glass`, `route`, `migrate`, `export-notifications`, and `provider-requests` support JSON or SQLite stores, repository routing files, claims-based local authorization, provider payload exports, and credential-free provider request specs.
...
Known gaps: full persistent API storage for every domain, live approval provider delivery, signed OIDC token validation, packaged Go backend, hosted sandbox deployment, and vendor-specific hooks beyond the MCP/CLI path.
