# CAVRA Customer Lifecycle Phase 8 Public Scorecard Monitoring Third-Cycle Activation Closeout

The customer lifecycle Phase 8 public scorecard monitoring third-cycle
activation closeout packet is the R7.45 release gate for proving that the
third public scorecard monitoring cycle has actually started. It closes out
...
The packet consumes the R7.44 public scorecard monitoring third-cycle readiness
...
acknowledgement refs, CI gate coverage, and explicit third-cycle activation
controls.
...
- The R7.44 public scorecard monitoring third-cycle readiness packet is live,
...
- The third-cycle activation contract includes source readiness, cycle start,
...
- Cycle-start refs prove the activation window, third-cycle start decision,
...
- CI gate coverage exists for source third-cycle readiness, cycle start,
  initial signal capture, monitor run, alert route check, evidence archive,
...
```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_activation_closeout.py \
  --export-dir examples/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-activation-closeout \
  --repo-root .
```

## Validate Third-Cycle Activation Closeout

```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_activation_closeout.py \
  --packet examples/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-activation-closeout/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-activation-closeout.live.sanitized.example.json \
  --repo-root . \
  --require-live
```
...
```json
{
  "ready_for_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_activation_closeout": true,
  "blocker_count": 0,
  "warning_count": 0
}
```
...
The public repository provides the third-cycle activation closeout contract,
