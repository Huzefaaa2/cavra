# CAVRA Customer Lifecycle Phase 8 Public Scorecard Monitoring Third-Cycle First Review

The customer lifecycle Phase 8 public scorecard monitoring third-cycle first
review packet is the R7.46 release gate for proving that the activated third
monitoring cycle has completed its first formal review window.

The packet consumes the R7.45 public scorecard monitoring third-cycle
...
third-cycle first-review controls.
...
- The R7.45 public scorecard monitoring third-cycle activation closeout packet
...
- The third-cycle first-review contract includes source activation closeout,
...
```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_first_review.py \
  --export-dir examples/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-first-review \
  --repo-root .
```

## Validate Third-Cycle First Review

```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_first_review.py \
  --packet examples/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-first-review/customer-lifecycle-phase8-public-scorecard-monitoring-third-cycle-first-review.live.sanitized.example.json \
  --repo-root . \
  --require-live
```
...
```json
{
  "ready_for_customer_lifecycle_phase8_public_scorecard_monitoring_third_cycle_first_review": true,
  "blocker_count": 0,
  "warning_count": 0
}
```
...
The public repository provides the third-cycle first-review contract,
