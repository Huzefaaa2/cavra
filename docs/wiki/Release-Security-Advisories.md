...
3. Confirm the package contains `checksums.txt`, `cavra-runtime.sbom.spdx.json`, `cavra-runtime.provenance.intoto.json`, detached signatures, `github-keyless-attestation.json`, and release evidence.
4. Verify locally with `cavra release verify-go-package go/cavra-runtime/dist/go-runtime-<version>`.
5. Verify the GitHub keyless attestation with `gh attestation verify go/cavra-runtime/dist/cavra-go-runtime-<version>.zip --repo Huzefaaa2/cavra`.
