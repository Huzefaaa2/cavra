...
Latest completed implementation phase: private Enterprise provider auth and rate-limit handling.

Next recommended implementation phase: add immutable object storage adapters for audit export bundles.
...
- Added private connector auth providers for OAuth client credentials, bearer tokens, API keys, and HTTP basic API-token flows.
- Added provider helper factories for Salesforce, HubSpot, Jira, ServiceNow, and Archer runtime auth.
- Added retryable rate-limit handling for provider `429`, `500`, `502`, `503`, and `504` responses with `Retry-After` support.
- Updated private HTTP workers so explicit auth providers can run without legacy endpoint bearer tokens.
- Added tests for static auth headers, OAuth token caching, auth-provider transport, and retryable rate-limit responses.
