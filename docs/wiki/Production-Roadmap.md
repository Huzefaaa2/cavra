...
Next recommended implementation phase: add provider-native OIDC token acquisition helpers for GitHub Actions, GitLab CI, and Azure Pipelines runner wrappers, plus production key custody and rotation documentation for runner authentication and daemon evidence verification keys.
...
- Add wrapper helpers that request OIDC tokens from GitHub Actions, GitLab CI, and Azure Pipelines provider metadata endpoints.
- Document deployment key custody, HMAC key rotation, JWKS trust configuration, and evidence verifier retention practices.
- Add CI examples that publish `release-governance-evidence-verification.json` as a required audit artifact.
