...
Next recommended implementation phase: add contract-level Go fixtures for the next high-risk release-governance metadata kinds, then document production release-signing operations and key rotation.
...
- Add contract-level fixtures for rollout evidence retrieval, promotion audit export, rollback audit export, and release connector delivery records.
