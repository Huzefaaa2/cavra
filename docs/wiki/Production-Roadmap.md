...
Next recommended implementation phase: add CI-provider OIDC token verification for runner authentication and verifier CLI support for daemon evidence streams.
...
- Add CI-provider OIDC verification for GitHub Actions, GitLab CI, and Azure Pipelines runner identities.
- Add CLI verification for daemon evidence stream signatures and hash chains.
- Document deployment key custody and rotation for runner auth and daemon evidence signing keys.
