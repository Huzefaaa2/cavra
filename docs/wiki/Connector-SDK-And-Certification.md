...
R4.1 is public-repository complete. The closeout boundary is documented in [CAVRA Connector SDK And Certification R4.1 Closeout](Connector-SDK-And-Certification-R4.1-Closeout.md). Provider-specific live sandbox logs, tenant credentials, partner onboarding evidence, support ownership records, and production connector run evidence remain R4.2 or deployment-specific evidence.
