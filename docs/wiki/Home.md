...
Phase 6, Console and Persistent API, has started. It now includes JSON and SQLite activity persistence for sessions and decisions, repository inventory and policy rollout persistence, persistent API backup/restore/retention operations, decision search filters, session summaries, and console Activity Explorer plus repository/rollout views.
