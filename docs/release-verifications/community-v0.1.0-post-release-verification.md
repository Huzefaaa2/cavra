...
Manual verification used the following public-safe command sequence:

```bash
rm -rf /tmp/cavra-v010-verify
mkdir -p /tmp/cavra-v010-verify
gh release download community-v0.1.0 --repo Huzefaaa2/cavra \
  --dir /tmp/cavra-v010-verify --pattern 'cavra-0.1.0*'
shasum -a 256 /tmp/cavra-v010-verify/*
python3 -m venv /tmp/cavra-v010-verify/venv
/tmp/cavra-v010-verify/venv/bin/python -m pip install --upgrade pip
/tmp/cavra-v010-verify/venv/bin/python -m pip install \
  /tmp/cavra-v010-verify/cavra-0.1.0-py3-none-any.whl
/tmp/cavra-v010-verify/venv/bin/cavra version
```
