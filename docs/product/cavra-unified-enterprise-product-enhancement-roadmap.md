...
| R2.1 | 2 | P1, P13 | Implement enterprise identity: OIDC/SAML, SCIM, RBAC, ABAC, break-glass, model-owner roles, and security-operator roles. | R1.4 | In Progress | Enterprise identity readiness contract, API endpoints, default policy, SAML bridge contract, SCIM lifecycle contract, RBAC/ABAC role model, and break-glass controls are implemented publicly; live IdP and private SCIM worker validation remain. | `python3 scripts/validate_enterprise_identity_readiness.py`; `python3 -m pytest tests/test_enterprise_identity.py tests/test_identity_references.py -q`. | `src/cavra/enterprise_identity.py`, `scripts/validate_enterprise_identity_readiness.py`, `docs/enterprise-identity-access-control.md`, `docs/oidc-rbac-deployment.md`, `/identity/enterprise-contract`, `/identity/enterprise-readiness`. |
