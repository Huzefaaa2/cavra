...
| Future roadmap | Closed unless product scope changes | Use the [roadmap intake gate](../roadmap-intake-gate.md) and [roadmap candidate charter](../roadmap-candidate-charter.md) before adding rows. Add new rows only for a new API, CLI command, validator family, connector, deployment target, AISPM capability, evidence schema, trust artifact, edition, packaging model, or buyer-facing surface. |
...
13. Keep repeated monitoring and customer-success cycles out of the public roadmap unless they create a new CAVRA product capability.
