...
This guide describes how to publish the public CAVRA Community service from
GitHub to Azure. It deploys:

- CAVRA FastAPI backend to Azure Container Apps.
- CAVRA static sandbox UI to Azure Static Web Apps.
...
isolation, private policy packs, live production connectors, SMTP report
delivery, license enforcement, or Enterprise AISPM production validation.
...
Azure Container Apps container filesystem storage is not a durable database.
For a Community SaaS demo, use explicit SQLite paths mounted on Azure Files or
accept that sample state is ephemeral. For production multi-tenant SaaS,
replace file-backed stores with an Enterprise-managed database design.
...
It does not claim Enterprise readiness. Enterprise readiness still requires
real tenant isolation, live connectors, report delivery, private policy packs,
