...
- Rollback drill notification history supports `connector_delivery_source`, `delivery_success`, `alert_level`, `audit_id`, `delivery_id`, and `cadence` filters; the dashboard includes acknowledgement audit delivery health, success rate, failed audit delivery count, destination provider summaries, retry-plan count, retryable count, retry acknowledgement count, retry execution approval count, live retry execution record counts, connector recovery closure counts, retry recovery report counts, recovery escalation route counts, recovery escalation acknowledgement counts, recovery escalation retry counts, recovery escalation retry health alert counts, recovery retry health alert delivery counts, recovery health alert retry-plan and retry-worker counts, recovery executive report counts, scheduled executive report counts, executive delivery retry plan counts, executive retry worker counts, executive retry execution outcomes, executive retry health counts, executive retry health alert delivery counts, executive health alert retry-plan and retry-worker counts, final closure dashboard state, release-readiness summary count, operator runbook export count, final readiness approval count, release record attachment count, closure packet verification count, verified closure packet count, auditor export count, final auditor export delivery count, final auditor export failed delivery count, final auditor export retry-plan count, final auditor export retryable count, immutable archive reference count, archive health report count, archive health alert count, worker run count, worker dry-run count, worker health alert count, and worker health acknowledgement count.
