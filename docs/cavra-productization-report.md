...
`go/cavra-runtime/` now contains the first Go enforcement-plane parity scaffold. It includes a Go module, runtime evaluator, command-line entrypoint, compiled-policy JSON loader, generated enforcement contracts, Unix-socket daemon transport, shared parity cases, and Go unit tests for critical file, command, Git, and MCP decisions. Python remains authoritative; the scaffold is intentionally bounded until client helpers, lifecycle management, evidence hooks, and binary release packaging are implemented.
