# CAVRA Customer Lifecycle Phase 8 Public Scorecard Monitoring Second-Cycle Activation Closeout

The customer lifecycle Phase 8 public scorecard monitoring second-cycle
activation closeout packet is the R7.41 release gate for proving that the
second public scorecard monitoring cycle has actually started. It closes out
...
The packet consumes the R7.40 public scorecard monitoring second-cycle readiness
...
acknowledgement refs, CI gate coverage, and explicit second-cycle activation
controls.
...
- The R7.40 public scorecard monitoring second-cycle readiness packet is live,
...
- The second-cycle activation contract includes source readiness, cycle start,
...
- Cycle-start refs prove the activation window, second-cycle start decision,
...
- CI gate coverage exists for source second-cycle readiness, cycle start,
  initial signal capture, monitor run, alert route check, evidence archive,
...
```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_activation_closeout.py \
  --export-dir examples/customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout \
  --repo-root .
```

## Validate Second-Cycle Activation Closeout

```bash
python3 scripts/validate_customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_activation_closeout.py \
  --packet examples/customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout/customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout.live.sanitized.example.json \
  --repo-root . \
  --require-live
```
...
```json
{
  "ready_for_customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_activation_closeout": true,
  "blocker_count": 0,
  "warning_count": 0
}
```
...
```bash
PYTHONPATH=src python3 -m cavra.cli release customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout \
  --packet examples/customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout/customer-lifecycle-phase8-public-scorecard-monitoring-second-cycle-activation-closeout.live.sanitized.example.json \
  --repo-root . \
  --require-live
```
...
The public repository provides the second-cycle activation closeout contract,
examples, validator, CLI command, tests, docs, and CI workflow. Real signal
payloads, monitor-run records, alert route internals, on-call rosters,
customer identities, secrets, tokens, pricing, schedules, and commercial
