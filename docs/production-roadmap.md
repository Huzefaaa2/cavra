...
Latest completed implementation phase: private Enterprise SSO claim binding for pilot-intake authorization, including configurable subject, tenant, email, role, group, issuer, audience, and group-to-role mappings.

Next recommended implementation phase: add customer/SaaS KMS envelope encryption for private pilot-intake payloads, then move storage to a managed tenant database and add real CRM/ITSM/GRC/customer-success connector workers.
...
- Added private `cavra_enterprise.identity` claim-binding helpers.
- Added configurable SSO claim mapping for subject, tenant, email, roles, groups, issuer, and audience.
- Added group-to-role mapping into Enterprise pilot roles.
- Added tenant mismatch, missing subject, and issuer/audience guardrails.
- Added tests proving bound SSO claims authorize tenant-scoped pilot intake updates.

Immediate next tasks:
- Add customer/SaaS KMS envelope encryption.
- Replace local private SQLite storage with managed tenant database storage.
- Implement live connector workers for CRM, ITSM, GRC, customer success, and tenant management.
