...
Next recommended implementation phase: add managed endpoint deployment reconciliation and drift monitoring for published CAVRA runtime versions.
...
- Capture desired versus observed runtime versions from managed endpoint tooling.
- Detect endpoints running stale, revoked, or unapproved runtime builds.
- Add API, CLI, and console views for deployment drift, reconciliation actions, and missed rollout targets.
