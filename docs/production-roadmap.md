...
Next recommended implementation phase: add daemon and CI runner examples for typed release governance enforcement requests.
...
- Add daemon JSON examples that send typed `release_governance` contract payloads through the Unix-socket transport.
- Add CI runner examples that validate release-governance evidence requests before promotion or rollback gates.
- Document how runner integrations should choose typed contract payloads over raw record maps.
