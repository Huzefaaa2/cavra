...
Latest completed implementation phase: provider-native private Enterprise adapters and immutable audit export/retention enforcement.

Next recommended implementation phase: add provider-specific authentication and rate-limit handling, then add immutable object storage adapters for audit export bundles.
...
- Added provider-native handoff payload adapters for Salesforce, HubSpot, Jira, ServiceNow, and Archer.
- Updated private HTTP workers so provider-native adapters can shape payloads while runtime endpoint/token handling stays separate.
- Added immutable audit export bundles with `events.json`, `manifest.json`, event checksum, manifest checksum, retention lock timestamp, and legal-hold flag.
- Added retention deletion checks for lock expiry and legal hold.
- Added tests for native payload shapes, HTTPS-preserving connector transport, immutable export manifests, checksum fields, retention locks, and legal-hold deletion blocking.

Immediate next tasks:
- Add provider-specific OAuth/API-key authentication flows and rate-limit handling.
