...
- SaaS Operating Automation Contract panel for inspecting the public-safe request, response, required checks, and private-service boundary.
- SaaS Worker Handoff panel for inspecting the public-safe handoff request, response, worker targets, scheduler references, evidence sink references, retry policy references, and private-service boundary.
...
When the API is available at the same origin, the console posts scenario runs to `/api/sandbox/run`, persists the resulting metadata through the API, loads public counters from `/api/sandbox/metrics`, loads `GET /evidence?limit=50`, loads artifact detail from `/evidence/{session_id}/artifacts`, reads `GET /saas/control-plane/contract`, posts `POST /saas/operating-automation` to preview the public-safe SaaS operating automation request/response handoff, and posts `POST /saas/operating-automation/worker-handoff` to preview the public-safe worker handoff request/response pair. If the API is not available, it uses built-in sample evidence metadata, sample scenario events, and local public-safe SaaS operating automation and worker handoff contract previews so the console remains usable as a static demo. The final closeout trial sample package is published at `evidence/final-closeout-trial/sample-evidence-package.json` in the Pages artifact.
