...
Implemented: CLI, MCP stdio server, FastAPI, webhook exporter, PR attestation exporter, Claude Code repository initializer, GitHub required-check workflow, GitHub Actions and GitLab CI enforcement templates, approval provider request specs, live approval provider delivery, SIEM export payloads, and JSON/SQLite integration inventory.

Reference or planned integrations: GitHub App orchestrator, Azure DevOps, pre-commit, VS Code, Docker, Kubernetes, Homebrew, PyPI, Sentinel, Splunk, Datadog, ServiceNow, Jira, Slack, Teams, Security Hub, CloudTrail, Azure Monitor, Google SCC, S3 Object Lock, Azure immutable blob, Entra ID, Okta, SAML, and RBAC.
