#!/usr/bin/env python3
"""Validate public-safe AISPM report operations readiness."""
...
PACKET_PATH = ROOT / "docs/release-verifications/aispm-report-operations-readiness.json"
...
def main() -> int:
    ...
    packet = load_json("docs/release-verifications/aispm-report-operations-readiness.json")
    require(
        packet.get("schema_version") == "cavra.aispm.report_operations_readiness.v1",
        f"{PACKET_PATH}: invalid schema_version",
        failures,
    )
    require(packet.get("status") == "ready", f"{PACKET_PATH}: status must be ready", failures)
    require(
        packet.get("portal_packet") == "cavra-aispm-report-operations-readiness-packet.json",
        f"{PACKET_PATH}: portal packet mismatch",
        failures,
    )
    operation_areas = packet.get("operation_areas", [])
    require(len(operation_areas) == 5, f"{PACKET_PATH}: expected 5 operation areas", failures)
    require(
        {area.get("area_id") for area in operation_areas}
        == {
        ...
        f"{PACKET_PATH}: operation area set mismatch",
        failures,
    )
    for area in operation_areas:
        ...
        require(
            ...
            f"{PACKET_PATH}: operation area must require Enterprise: {area}",
            failures,
        )
    require(
    ...
    require(
        ...
        "release evidence index missing report operations readiness item",
        failures,
    )
    ...
    require(
        ...
        "launch readiness rollup missing report operations readiness source",
        failures,
    )
    ...
    for needle in [
        ...
        "aispmReportOperationsReadinessItems",
    ...
    for needle in [
        ...
        'grep -q "cavra-aispm-report-operations-readiness-packet.json"',
    ]:
    ...
    combined = "\n".join(
        [
            read("docs/release-verifications/aispm-report-operations-readiness.json"),
            read("docs/release-verifications/aispm-report-operations-readiness.md"),
            html,
            js,
        ]
    )
    for term in forbidden:
        require(term not in combined, f"report operations readiness must not expose {term}", failures)

    if failures:
        print("AISPM report operations readiness validation failed:")
        for failure in failures:
            print(f"- {failure}")
        return 1

    print("AISPM report operations readiness validation passed.")
    return 0
