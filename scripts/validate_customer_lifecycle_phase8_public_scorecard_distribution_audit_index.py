...
from cavra.customer_lifecycle_phase8_public_scorecard_distribution_audit_index import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard distribution audit index.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard distribution audit index packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_distribution_audit_index_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_distribution_audit_index", False):
        return 1
