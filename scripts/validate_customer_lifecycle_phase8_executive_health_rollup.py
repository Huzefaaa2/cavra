...
from cavra.customer_lifecycle_phase8_executive_health_rollup import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 executive health rollup.")
    parser.add_argument("--packet", type=Path, help="Optional executive health rollup packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_executive_health_rollup_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
        ...
        result = validate_customer_lifecycle_phase8_executive_health_rollup_packet(packet, require_live=args.require_live)
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_executive_health_rollup", False):
        return 1
