...
from cavra.managed_enterprise_stabilization_report import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA Managed/Enterprise stabilization reports.")
    parser.add_argument("--report", type=Path, help="Managed/Enterprise stabilization report JSON.")
    parser.add_argument("--export-dir", type=Path, help="Export sample and live sanitized stabilization report templates.")
    ...
    if args.export_dir:
        result = write_managed_enterprise_stabilization_report_artifacts(args.export_dir)
        exit_ok = result["ready_for_managed_enterprise_stabilization_closeout"] is True
    else:
        if args.report:
            report = json.loads(args.report.read_text(encoding="utf-8"))
        else:
            report = build_managed_enterprise_stabilization_report(
                evidence_mode="live" if args.require_live else "sample",
            )
        result = validate_managed_enterprise_stabilization_report(report, require_live=args.require_live)
