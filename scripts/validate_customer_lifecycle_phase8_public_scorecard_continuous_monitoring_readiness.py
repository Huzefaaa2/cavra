...
from cavra.customer_lifecycle_phase8_public_scorecard_continuous_monitoring_readiness import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard continuous monitoring readiness.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard continuous monitoring readiness packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_continuous_monitoring_readiness_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_continuous_monitoring_readiness", False):
        return 1
