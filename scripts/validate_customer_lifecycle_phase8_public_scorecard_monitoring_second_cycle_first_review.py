...
from cavra.customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_first_review import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard monitoring second-cycle first review.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard monitoring second-cycle first review packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_first_review_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_monitoring_second_cycle_first_review", False):
        return 1
