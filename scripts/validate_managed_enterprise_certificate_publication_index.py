...
from cavra.managed_enterprise_certificate_publication_index import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Validate CAVRA Managed/Enterprise certificate publication indexes.",
    )
    parser.add_argument("--index", type=Path, help="Managed/Enterprise certificate publication index JSON.")
    parser.add_argument(
        "--export-dir",
        type=Path,
        help="Export sample and live sanitized certificate publication index templates.",
    )
    ...
    if args.export_dir:
        result = write_managed_enterprise_certificate_publication_index_artifacts(args.export_dir)
        exit_ok = result["ready_for_managed_enterprise_certificate_publication"] is True
    else:
        if args.index:
            index = json.loads(args.index.read_text(encoding="utf-8"))
        else:
            index = build_managed_enterprise_certificate_publication_index(
                evidence_mode="live" if args.require_live else "sample",
            )
        result = validate_managed_enterprise_certificate_publication_index(index, require_live=args.require_live)
