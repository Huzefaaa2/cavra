...
from cavra.customer_lifecycle_phase8_public_scorecard_publication_closeout import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard publication closeout.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard publication closeout packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_publication_closeout_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_publication_closeout", False):
        return 1
