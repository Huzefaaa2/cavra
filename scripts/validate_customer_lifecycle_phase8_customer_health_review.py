...
from cavra.customer_lifecycle_phase8_customer_health_review import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 customer health review.")
    parser.add_argument("--packet", type=Path, help="Optional customer health review packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_customer_health_review_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
        ...
        result = validate_customer_lifecycle_phase8_customer_health_review_packet(packet, require_live=args.require_live)
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_customer_health_review", False):
        return 1
