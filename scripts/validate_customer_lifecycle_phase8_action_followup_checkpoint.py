...
from cavra.customer_lifecycle_phase8_action_followup_checkpoint import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 action follow-up checkpoint.")
    parser.add_argument("--packet", type=Path, help="Optional action follow-up checkpoint packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_action_followup_checkpoint_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
        ...
        result = validate_customer_lifecycle_phase8_action_followup_checkpoint_packet(packet, require_live=args.require_live)
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_action_followup_checkpoint", False):
        return 1
