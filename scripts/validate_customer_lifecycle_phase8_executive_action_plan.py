...
from cavra.customer_lifecycle_phase8_executive_action_plan import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 executive action plan.")
    parser.add_argument("--packet", type=Path, help="Optional executive action plan packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_executive_action_plan_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
        ...
        result = validate_customer_lifecycle_phase8_executive_action_plan_packet(packet, require_live=args.require_live)
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_executive_action_plan", False):
        return 1
