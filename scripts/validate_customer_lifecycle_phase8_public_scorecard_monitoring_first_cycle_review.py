...
from cavra.customer_lifecycle_phase8_public_scorecard_monitoring_first_cycle_review import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard monitoring first-cycle review.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard monitoring first-cycle review packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_monitoring_first_cycle_review_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_monitoring_first_cycle_review", False):
        return 1
