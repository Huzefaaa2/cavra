...
from cavra.customer_lifecycle_phase8_public_scorecard_audit_review_closeout import (  # noqa: E402
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Validate CAVRA customer lifecycle Phase 8 public scorecard audit review closeout.")
    parser.add_argument("--packet", type=Path, help="Optional public scorecard audit review closeout packet JSON.")
    ...
    if args.export_dir:
        result = write_customer_lifecycle_phase8_public_scorecard_audit_review_closeout_artifacts(args.export_dir, args.repo_root)
    else:
        if args.packet:
    ...
    print(rendered)
    if args.require_live and not result.get("ready_for_customer_lifecycle_phase8_public_scorecard_audit_review_closeout", False):
        return 1
