...
def test_api_exposes_aispm_dashboard_contract_and_local_posture(monkeypatch, tmp_path) -> None:
    ...
    assert policy_context_gaps.status_code == 200
    assert policy_context_gaps.json()["schema_version"] == "cavra.aispm.policy_context_gaps.v1"
    ...
    assert evidence_freshness.status_code == 200
    assert evidence_freshness.json()["schema_version"] == "cavra.aispm.evidence_freshness.v1"
    assert evidence_freshness.json()["summary"]["total_items"] == 1
    assert evidence_freshness.json()["items"][0]["agent_id"] == "codex-agent"
    assert evidence_freshness.json()["redaction"]["object_lock_status"] == "requires_cavra_enterprise"
    assert executive_risk_narrative.status_code == 200
    assert executive_risk_narrative.json()["schema_version"] == "cavra.aispm.executive_risk_narrative.v1"
    assert "CAVRA Community reports" in executive_risk_narrative.json()["narrative"]["headline"]
    assert executive_risk_narrative.json()["narrative"]["key_metrics"]["blocked_actions"] == 1
    assert executive_risk_narrative.json()["redaction"]["ai_generated_board_summary"] == "requires_cavra_enterprise"
    assert replay_to_policy.status_code == 200
    assert replay_to_policy.json()["schema_version"] == "cavra.aispm.replay_to_policy_draft.v1"
    assert replay_to_policy.json()["summary"]["recommended_rules"] == 1
    assert replay_to_policy.json()["summary"]["draft_valid"] is True
...
def test_api_saas_operating_automation_worker_handoff_surface(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("CAVRA_EVIDENCE_METADATA_STORE", str(tmp_path / "metadata.json"))
    client = TestClient(create_app())
    ...
    assert config["endpoints"]["saas_operating_automation_worker_handoff"] == (
        "/saas/operating-automation/worker-handoff"
    )
    assert response.status_code == 200
    payload = response.json()
    ...
    assert payload["response"]["payload"]["summary"]["worker_targets"] == [
        "billing_monitoring",
        "support_followup",
    ]


def test_api_saas_operating_automation_worker_handoff_rejects_sensitive_values(monkeypatch, tmp_path) -> None:
    monkeypatch.setenv("CAVRA_EVIDENCE_METADATA_STORE", str(tmp_path / "metadata.json"))
    client = TestClient(create_app())
...
def test_api_deployment_production_readiness(monkeypatch, tmp_path) -> None:
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_acknowledge"]
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_plan"]
        == "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-plan"
    )
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_worker"]
        == "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-worker-run"
    )
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_health"]
        == "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-health"
    )
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_deliver"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_health_alerts"
        ]
        == "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-health-alerts"
    )
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_dashboard"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_retry_plan"
        ]
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_schedule_run"]
        == "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-executive-report/schedule-run"
    )
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_deliver"]
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_plan"]
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_worker"]
    ...
    assert (
        config["endpoints"]["go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_health"]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_health_alert_retry_plan"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_health_alert_retry_worker"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_final_reporting_closure_dashboard"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_final_reporting_release_readiness_approval"
        ]
    ...
    assert (
        config["endpoints"][
            "go_rollback_drill_notification_acknowledgement_audit_delivery_final_reporting_release_record_attachment"
        ]
...
def test_api_go_backend_rollback_drill_notification_delivery(monkeypatch, tmp_path) -> None:
    ...
    audit_delivery_recovery_escalation_ack = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        f"recovery-escalations/{audit_delivery_recovery_escalation_plan.json()['plan']['plan_id']}/acknowledgements",
        json={
            "provider": "webhook",
            "acknowledged_by": "release-manager",
            "acknowledgement_state": "accepted",
    ...
    audit_delivery_recovery_escalation_retry_plan = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-plan",
        json={
            "generated_by": "release-manager",
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
    ...
    audit_delivery_recovery_escalation_retry_worker = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-worker-run",
        json={
            "generated_by": "release-manager",
            "execute": True,
            "max_retry_deliveries": 2,
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
            "schedule": {"interval_minutes": 30, "cadence": "manual_recovery_escalation_retry"},
            "retries": 0,
            "timeout_seconds": 0.1,
        },
    )
    audit_delivery_recovery_escalation_retry_health = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-escalations/retry-health",
        params={"expected_interval_minutes": 30, "stale_metadata_minutes": 120, "generated_by": "release-manager"},
    )
    audit_delivery_recovery_escalation_retry_health_alert_delivery = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-escalations/retry-health-alerts/deliver",
        json={
            "generated_by": "release-manager",
            "provider": "webhook",
            "force": True,
    ...
    recovery_retry_health_alert_plan = audit_delivery_recovery_escalation_retry_health_alert_delivery.json().get(
        "plan", {}
    )
    audit_delivery_recovery_escalation_retry_health_alert_retry_plan = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-escalations/retry-health-alerts/retry-plan",
        json={
            "generated_by": "release-manager",
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
        },
    )
    audit_delivery_recovery_escalation_retry_health_alert_retry_worker = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-escalations/retry-health-alerts/retry-worker-run",
        json={
            "generated_by": "release-manager",
            "execute": True,
            "max_retry_deliveries": 2,
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
            "schedule": {"interval_minutes": 30, "cadence": "manual_recovery_health_alert_retry"},
            "retries": 0,
            "timeout_seconds": 0.1,
        },
    )
    audit_delivery_recovery_escalation_retry_health_alert_ack = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        f"recovery-escalations/retry-health-alerts/{recovery_retry_health_alert_plan.get('health_id', 'missing')}/acknowledgements",
        json={
            "provider": "webhook",
            "acknowledged_by": "release-manager",
            "acknowledgement_state": "acknowledged",
            "plan_id": recovery_retry_health_alert_plan.get("plan_id"),
        },
    )
    audit_delivery_recovery_escalation_retry_health_alert_history = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-escalations/retry-health-alerts"
    )
    audit_delivery_recovery_escalation_retry_health_alert_dashboard = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-escalations/retry-health-alert-dashboard"
    )
    audit_delivery_recovery_executive_report = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-executive-report",
        params={"recovery_slo_minutes": 120, "generated_by": "release-manager"},
    )
    audit_delivery_recovery_executive_schedule_run = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-executive-report/schedule-run",
        json={
            "generated_by": "release-manager",
    ...
    audit_delivery_recovery_executive_schedule_delivery = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        f"recovery-executive-report/schedule-runs/{audit_delivery_recovery_executive_schedule_run.json()['run']['run_id']}/deliver",
        json={
            "generated_by": "release-manager",
            "provider": "webhook",
            "retries": 0,
    ...
    audit_delivery_recovery_executive_delivery_retry_plan = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/recovery-executive-report/delivery-retry-plan",
        json={
            "generated_by": "release-manager",
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
    ...
    audit_delivery_recovery_executive_delivery_retry_worker = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        ...
        json={
            ...
            "max_retry_deliveries": 2,
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
            "schedule": {"interval_minutes": 30, "cadence": "manual_executive_delivery_retry"},
            "retries": 0,
            "timeout_seconds": 0.1,
        },
    )
    audit_delivery_recovery_executive_delivery_retry_health = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health",
        params={"expected_interval_minutes": 60, "stale_metadata_minutes": 180, "generated_by": "release-manager"},
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_delivery = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health-alerts/deliver",
        json={
            "generated_by": "release-manager",
            "provider": "webhook",
            "force": True,
    ...
    executive_retry_health_alert_plan = audit_delivery_recovery_executive_delivery_retry_health_alert_delivery.json().get(
        "plan", {}
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_retry_plan = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health-alerts/retry-plan",
        json={
            "generated_by": "release-manager",
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
        },
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_retry_worker = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health-alerts/retry-worker-run",
        json={
            "generated_by": "release-manager",
            "execute": True,
            "max_retry_deliveries": 2,
            "retry_policy": {"max_retry_attempts": 3, "retry_delay_minutes": 0, "allow_immediate_retry": True},
            "schedule": {"interval_minutes": 30, "cadence": "manual_executive_health_alert_retry"},
            "retries": 0,
            "timeout_seconds": 0.1,
        },
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_ack = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        f"recovery-executive-report/delivery-retry-health-alerts/{executive_retry_health_alert_plan.get('health_id', 'missing')}/acknowledgements",
        json={
            "provider": "webhook",
            "acknowledged_by": "release-manager",
            "acknowledgement_state": "acknowledged",
            "plan_id": executive_retry_health_alert_plan.get("plan_id"),
        },
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_history = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health-alerts"
    )
    audit_delivery_recovery_executive_delivery_retry_health_alert_dashboard = client.get(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        "recovery-executive-report/delivery-retry-health-alert-dashboard"
    )
    ...
    final_reporting_release_readiness_approval = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/"
        f"final-reporting-release-readiness/{final_reporting_release_readiness.json()['summary']['summary_id']}/approval-decisions",
        json={
            "decided_by": "release-manager",
            "approval_state": "approved",
            "override_blockers": True,
            "external_ref": "REL-123",
            "notes": "Approved for release record attachment after governance review.",
        },
    )
    final_reporting_release_record_attachment = client.post(
        "/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-record-attachment",
        json={
            "attached_by": "release-manager",
            "approval_decision_id": final_reporting_release_readiness_approval.json()["decision"]["decision_id"],
    ...
    final_reporting_signed_archive_manifest = client.post(
        ...
        json={
            ...
            "signature_key_id": "test-release-key",
        },
    )
    final_reporting_release_closeout_summary = client.get(
    ...
    assert audit_delivery_recovery_escalation_ack.status_code == 200
    assert (
        audit_delivery_recovery_escalation_ack.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-ack"
    )
    assert audit_delivery_recovery_escalation_retry_plan.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_plan.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-delivery-retry-plan"
    )
    assert audit_delivery_recovery_escalation_retry_plan.json()["plan"]["retryable_count"] == 1
    assert audit_delivery_recovery_escalation_retry_worker.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_worker.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-worker-run"
    )
    assert audit_delivery_recovery_escalation_retry_worker.json()["run"]["dry_run"] is False
    assert (
        audit_delivery_recovery_escalation_retry_worker.json()["retry_results"][0]["execution_metadata"][
            "metadata_kind"
        ]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-execution-record"
    )
    assert audit_delivery_recovery_escalation_retry_health.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_health.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-health"
    )
    assert audit_delivery_recovery_escalation_retry_health.json()["health"]["alert_count"] >= 1
    assert audit_delivery_recovery_escalation_retry_health_alert_delivery.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_health_alert_delivery.json()["plan_metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-health-alert-plan"
    )
    assert audit_delivery_recovery_escalation_retry_health_alert_retry_plan.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_health_alert_retry_plan.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-health-alert-delivery-retry-plan"
    )
    assert audit_delivery_recovery_escalation_retry_health_alert_retry_plan.json()["plan"]["retryable_count"] == 1
    assert audit_delivery_recovery_escalation_retry_health_alert_retry_worker.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_health_alert_retry_worker.json()["metadata"]["metadata_kind"]
    ...
    assert audit_delivery_recovery_escalation_retry_health_alert_retry_worker.json()["run"]["dry_run"] is False
    assert (
        audit_delivery_recovery_escalation_retry_health_alert_retry_worker.json()["retry_results"][0][
            "execution_metadata"
        ]["metadata_kind"]
    ...
    assert audit_delivery_recovery_escalation_retry_health_alert_ack.status_code == 200
    assert (
        audit_delivery_recovery_escalation_retry_health_alert_ack.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-escalation-retry-health-alert-ack"
    )
    assert audit_delivery_recovery_escalation_retry_health_alert_history.status_code == 200
    assert audit_delivery_recovery_escalation_retry_health_alert_history.json()["total"] >= 2
    assert audit_delivery_recovery_escalation_retry_health_alert_dashboard.status_code == 200
    assert audit_delivery_recovery_escalation_retry_health_alert_dashboard.json()["acknowledgement_count"] == 1
    ...
    assert audit_delivery_recovery_executive_schedule_run.status_code == 200
    assert (
        audit_delivery_recovery_executive_schedule_run.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-schedule-run"
    )
    assert audit_delivery_recovery_executive_schedule_delivery.status_code == 200
    assert (
        audit_delivery_recovery_executive_schedule_delivery.json()["metadata"]["connector_delivery_source"]
        == "go_backend_rollback_drill_acknowledgement_audit_recovery_executive_report"
    )
    assert audit_delivery_recovery_executive_delivery_retry_plan.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_plan.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-plan"
    )
    assert audit_delivery_recovery_executive_delivery_retry_plan.json()["plan"]["retryable_count"] == 1
    assert audit_delivery_recovery_executive_delivery_retry_worker.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_worker.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-worker-run"
    )
    assert audit_delivery_recovery_executive_delivery_retry_worker.json()["run"]["dry_run"] is False
    assert (
        audit_delivery_recovery_executive_delivery_retry_worker.json()["retry_results"][0]["execution_metadata"][
            "metadata_kind"
        ]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-execution-record"
    )
    assert audit_delivery_recovery_executive_delivery_retry_health.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_health.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-health"
    )
    assert audit_delivery_recovery_executive_delivery_retry_health.json()["health"]["alert_count"] >= 1
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_delivery.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_delivery.json()["plan_metadata"][
            "metadata_kind"
        ]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-health-alert-plan"
    )
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_delivery.json()["metadata"][
    ...
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_retry_plan.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_retry_plan.json()["metadata"]["metadata_kind"]
    ...
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_retry_plan.json()["plan"]["retryable_count"] == 1
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_retry_worker.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_retry_worker.json()["metadata"][
            "metadata_kind"
        ]
    ...
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_retry_worker.json()["run"]["dry_run"] is False
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_retry_worker.json()["retry_results"][0][
            "execution_metadata"
        ]["metadata_kind"]
    ...
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_ack.status_code == 200
    assert (
        audit_delivery_recovery_executive_delivery_retry_health_alert_ack.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-recovery-executive-report-delivery-retry-health-alert-ack"
    )
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_history.status_code == 200
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_history.json()["total"] >= 2
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_dashboard.status_code == 200
    assert audit_delivery_recovery_executive_delivery_retry_health_alert_dashboard.json()["acknowledgement_count"] == 1
    assert history.status_code == 200
    assert history.json()["total"] >= 42
    ...
    assert final_reporting_closure_dashboard.status_code == 200
    ...
    assert final_reporting_release_readiness_approval.status_code == 200
    assert (
        final_reporting_release_readiness_approval.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-approval-decision"
    )
    assert final_reporting_release_readiness_approval.json()["decision"]["override_blockers"] is True
    assert final_reporting_release_record_attachment.status_code == 200
    assert (
        final_reporting_release_record_attachment.json()["metadata"]["metadata_kind"]
    ...
    assert final_reporting_readiness_bundle.status_code == 200
    assert (
        final_reporting_readiness_bundle.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-readiness-bundle"
    )
    assert (
        final_reporting_readiness_bundle.json()["bundle"]["closeout_state"] == "ready_for_closeout"
    ), final_reporting_readiness_bundle.json()["bundle"]["blockers"]
    assert final_reporting_readiness_bundle.json()["bundle"]["archive_object_count"] >= 1
    assert final_reporting_signed_archive_manifest.status_code == 200
    assert (
        final_reporting_signed_archive_manifest.json()["metadata"]["metadata_kind"]
        == "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-signed-archive-manifest"
    )
    assert final_reporting_signed_archive_manifest.json()["manifest"]["signature_state"] == "external_signature_attached"
    assert final_reporting_release_closeout_summary.status_code == 200
    assert (
        final_reporting_release_closeout_summary.json()["metadata"]["metadata_kind"]
    ...
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_ack_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_plan_count"] == 2
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retryable_count"] == 2
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_worker_run_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_execution_record_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_execution_failed_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_health_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_count"] >= 1
    assert (
        dashboard_after.json()[
            "acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_plan_count"
        ]
        == 1
    )
    assert (
        dashboard_after.json()[
            "acknowledgement_audit_delivery_recovery_escalation_retry_health_alert_ack_count"
        ]
        == 1
    )
    assert (
    ...
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_count"] == 2
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_schedule_run_count"] == 1
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_delivery_count"] == 2
    assert (
        dashboard_after.json()["failed_acknowledgement_audit_delivery_recovery_executive_report_delivery_count"] == 2
    )
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_plan_count"] == 2
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_delivery_retryable_count"] == 2
    assert (
        dashboard_after.json()[
            "acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_worker_run_count"
        ]
        == 1
    )
    assert (
        dashboard_after.json()[
            "acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_execution_record_count"
        ]
        == 1
    )
    assert (
        dashboard_after.json()[
            "acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_execution_failed_count"
        ]
        == 1
    )
    assert dashboard_after.json()["acknowledgement_audit_delivery_recovery_executive_report_delivery_retry_health_count"] == 1
    assert (
        dashboard_after.json()[
