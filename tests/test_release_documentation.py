...
def test_community_v011_maintenance_release_is_linked_and_validated() -> None:
    ...
    assert "official maintenance release" in verification_json["next_recommendation"]
...
def test_community_v012_readiness_is_linked_and_validated() -> None:
    ...
    for document in (readiness, wiki_readiness):
        ...
        assert "official maintenance release" in document
...
def test_community_v012_dry_run_release_is_linked_and_validated() -> None:
    ...
    assert "Community v0.1.2 dry-run release record:" in inventory
    assert "v0.1.2 dry-run release notes" in changelog

    for document in (release_notes, verification, wiki_release_notes, wiki_verification):
        assert release_url in document
        assert "dry run" in document.lower() or "dry-run" in document.lower()
        assert "official maintenance release" in document
        assert "Enterprise source code" in document

    assert release_notes_path in release_index
    assert verification_path in release_index
    assert "| Community v0.1.2 | Dry run |" in release_index
    ...
    assert gate_statuses["Artifact checksums"] == "warn"
    assert gate_statuses["Install smoke"] == "warn"
