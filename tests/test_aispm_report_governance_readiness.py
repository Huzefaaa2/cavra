...
def test_aispm_report_governance_readiness_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate-aispm-report-governance-readiness.py"],
        check=False,
        text=True,
    ...
    assert "AISPM report governance readiness validation passed." in result.stdout


def test_aispm_report_governance_readiness_contract() -> None:
    packet = json.loads(
        Path("docs/release-verifications/aispm-report-governance-readiness.json").read_text(
            encoding="utf-8"
        )
    )

    assert packet["schema_version"] == "cavra.aispm.report_governance_readiness.v1"
    assert packet["status"] == "ready"
    assert packet["portal_packet"] == "cavra-aispm-report-governance-readiness-packet.json"
    assert packet["validator"] == "scripts/validate-aispm-report-governance-readiness.py"
    assert {area["area_id"] for area in packet["governance_areas"]} == {
        "schedule_policy",
        "recipient_policy",
        "approval_decisions",
        "exception_lifecycle",
    ...
    assert "recipient addresses" in packet["public_safety_boundary"]


def test_aispm_report_governance_is_wired_into_portal_workflows_and_docs() -> None:
    command = "python scripts/validate-aispm-report-governance-readiness.py"
    workflow_paths = [
