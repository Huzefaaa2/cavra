...
def test_production_readiness_report_marks_missing_controls() -> None:
    ...
    assert any(item["id"] == "go_backend_rollback_drill_history" and item["status"] == "pass" for item in report["checks"])
    assert any(item["id"] == "go_backend_rollback_drill_schedule" and item["status"] == "pass" for item in report["checks"])
    ...
    assert report["go_backend_rollback_drill_history"]["status"] == "not_requested"
    assert report["go_backend_rollback_drill_schedule"]["status"] == "not_requested"
