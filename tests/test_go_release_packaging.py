...
def test_go_release_packaging_creates_sbom_checksums_and_evidence(tmp_path: Path) -> None:
    ...
    signing_operations = json.loads((dist / "cavra-runtime.signing-operations.json").read_text(encoding="utf-8"))
    ...
    assert "cavra-runtime.signing-operations.json" in checksums
    ...
    assert "cavra-runtime.signing-operations.json" in bootstrap["required_files"]
    ...
    assert signing_operations["schema_version"] == "cavra.go-runtime.signing-operations.v1"
    assert signing_operations["component"] == "go-enforcement-plane"
    assert signing_operations["active_key_id"] == "cavra-go-release"
    assert signing_operations["algorithm"] == "Ed25519"
    assert signing_operations["signing_required"] is False
    assert "operator-managed-secret-store" in signing_operations["custody_model"]["private_key_location"]
    assert "private signing keys must never be committed" in signing_operations["custody_model"][
        "private_key_public_repo_boundary"
    ]
    assert "quarterly" in signing_operations["rotation_policy"]["cadence"]
    assert "incident" in signing_operations["rotation_policy"]["cadence"]
    assert "revoked key_id" in signing_operations["emergency_revocation"]["required_evidence"]
    assert "replacement key_id" in signing_operations["emergency_revocation"]["required_evidence"]
    assert "customer advisory reference" in signing_operations["emergency_revocation"]["required_evidence"]
    assert "disable the compromised signing secret" in signing_operations["emergency_revocation"]["required_actions"]
    assert "rebuild and sign replacement packages with a new key_id" in signing_operations["emergency_revocation"][
        "required_actions"
    ]
    assert set(signing_operations["controls"]) >= {
    ...
    assert all(
        ".cavra/go-daemon/release-governance-evidence-verification.json" in bundle["required_outputs"]
        for bundle in ci_runner_bundles["runner_bundles"]
    )
...
def test_go_release_verifier_accepts_signed_package_and_rejects_tampering(tmp_path: Path, monkeypatch) -> None:
    ...
    assert "cavra-runtime.signing-operations.json" in valid_result.verified_artifacts
    assert "cavra-runtime.signing-operations.json" in valid_result.verified_provenance
    assert "cavra-runtime.signing-operations.json" in valid_result.verified_signatures
...
def test_go_release_verifier_rejects_missing_signing_operations_manifest(tmp_path: Path, monkeypatch) -> None:
    private_key = tmp_path / "keys" / "private.pem"
    public_key = tmp_path / "keys" / "public.pem"
    generate_ed25519_keypair(private_key, public_key)
    monkeypatch.setenv("CAVRA_GO_RELEASE_SIGNING_KEY", private_key.read_text(encoding="utf-8"))
    ...
    (dist / "cavra-runtime.signing-operations.json").unlink()

    invalid_result = verify_go_release_package(dist)
    assert not invalid_result.valid
    assert any("missing cavra-runtime.signing-operations.json" in error for error in invalid_result.errors)
...
def test_endpoint_drift_remediation_requires_approval_and_indexes_execution(
    ...
    failed_recurrence_delivery_metadata = {
        ...
        "created_at": "2026-05-19T01:20:00+00:00",
    ...
    automation_now = datetime(2026, 5, 19, 1, 35, tzinfo=timezone.utc)
    recurrence_automation_run = build_endpoint_remediation_sla_escalation_recurrence_automation_run(
        [recurrence_metadata, suppression_audit_metadata, failed_recurrence_delivery_metadata],
        retry_policy={"max_retry_attempts": 3, "retry_delay_minutes": 1, "backoff_multiplier": 1},
        schedule={"interval_minutes": 30},
        dry_run=True,
        now=automation_now,
    )
    repeated_recurrence_automation_run = build_endpoint_remediation_sla_escalation_recurrence_automation_run(
        [recurrence_metadata, suppression_audit_metadata, failed_recurrence_delivery_metadata],
        retry_policy={"max_retry_attempts": 3, "retry_delay_minutes": 1, "backoff_multiplier": 1},
        schedule={"interval_minutes": 30},
        dry_run=True,
        now=automation_now + timedelta(minutes=2),
    )
    recurrence_automation_metadata = build_endpoint_remediation_sla_escalation_recurrence_automation_run_metadata(
        recurrence_automation_run
    )
    ...
    assert (
        recurrence_automation_metadata["metadata_kind"]
        == "endpoint-remediation-sla-escalation-recurrence-automation-run"
    )
    assert recurrence_automation_run["dry_run"] is True
    assert repeated_recurrence_automation_run["run_id"] == recurrence_automation_run["run_id"]
    assert recurrence_automation_run["summary"]["retryable_count"] >= 1
    ...
    sla_escalation_recurrence_automation_cli = runner.invoke(
        app,
        [
            "release",
            "endpoint-remediation-sla-escalation-recurrence-automation",
            "--metadata-json",
            str(metadata_json),
    ...
    sla_escalation_recurrence_automation_history_cli = runner.invoke(
        app,
        [
            "release",
            "endpoint-remediation-sla-escalation-recurrence-automation-history",
            "--metadata-json",
            str(metadata_json),
        ],
    )
    sla_escalation_recurrence_automation_dashboard_cli = runner.invoke(
        app,
        [
            "release",
            "endpoint-remediation-sla-escalation-recurrence-automation-dashboard",
            "--metadata-json",
            str(metadata_json),
        ],
    )
    ...
    assert sla_escalation_recurrence_automation_cli.exit_code == 0
    assert (
        json.loads(sla_escalation_recurrence_automation_cli.output)["metadata"]["metadata_kind"]
        == "endpoint-remediation-sla-escalation-recurrence-automation-run"
    )
    assert json.loads(sla_escalation_recurrence_automation_cli.output)["run"]["dry_run"] is True
    assert sla_escalation_recurrence_automation_history_cli.exit_code == 0
