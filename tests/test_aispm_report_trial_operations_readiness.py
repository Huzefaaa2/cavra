...
def test_aispm_report_trial_operations_readiness_validator_passes() -> None:
    result = subprocess.run(
        [sys.executable, "scripts/validate-aispm-report-trial-operations-readiness.py"],
        check=False,
        text=True,
    ...
    assert "AISPM report trial operations readiness validation passed." in result.stdout


def test_aispm_report_trial_operations_readiness_contract() -> None:
    packet = json.loads(
        Path("docs/release-verifications/aispm-report-trial-operations-readiness.json").read_text(
            encoding="utf-8"
        )
    )

    assert packet["schema_version"] == "cavra.aispm.report_trial_operations_readiness.v1"
    assert packet["status"] == "ready"
    assert packet["portal_packet"] == "cavra-aispm-report-trial-operations-readiness-packet.json"
    assert packet["validator"] == "scripts/validate-aispm-report-trial-operations-readiness.py"
    assert {area["area_id"] for area in packet["trial_operations_areas"]} == {
    ...
    assert "license keys" in packet["public_safety_boundary"]


def test_aispm_report_trial_operations_is_wired_into_portal_workflows_and_docs() -> None:
    command = "python scripts/validate-aispm-report-trial-operations-readiness.py"
    workflow_paths = [
