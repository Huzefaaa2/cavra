...
function goDrillNotificationPayload(item) {
  ...
  if (item?.final_reporting_release_readiness_approval_decision && typeof item.final_reporting_release_readiness_approval_decision === "object") return item.final_reporting_release_readiness_approval_decision;
  if (item?.final_reporting_release_record_attachment && typeof item.final_reporting_release_record_attachment === "object") return item.final_reporting_release_record_attachment;
  if (item?.final_reporting_release_closure_packet_verification && typeof item.final_reporting_release_closure_packet_verification === "object") return item.final_reporting_release_closure_packet_verification;
  if (item?.final_reporting_auditor_export && typeof item.final_reporting_auditor_export === "object") return item.final_reporting_auditor_export;
  if (item?.final_reporting_auditor_export_delivery_retry_plan && typeof item.final_reporting_auditor_export_delivery_retry_plan === "object") return item.final_reporting_auditor_export_delivery_retry_plan;
  if (item?.final_reporting_auditor_export_delivery_retry_worker_run && typeof item.final_reporting_auditor_export_delivery_retry_worker_run === "object") return item.final_reporting_auditor_export_delivery_retry_worker_run;
  if (item?.final_reporting_auditor_export_delivery_retry_execution_record && typeof item.final_reporting_auditor_export_delivery_retry_execution_record === "object") return item.final_reporting_auditor_export_delivery_retry_execution_record;
  ...
  if (item?.final_reporting_closeout_retention_health && typeof item.final_reporting_closeout_retention_health === "object") return item.final_reporting_closeout_retention_health;
  if (item?.final_reporting_closeout_retention_health_alert_plan && typeof item.final_reporting_closeout_retention_health_alert_plan === "object") return item.final_reporting_closeout_retention_health_alert_plan;
  if (item?.final_reporting_release_closeout_delivery_retry_plan && typeof item.final_reporting_release_closeout_delivery_retry_plan === "object") return item.final_reporting_release_closeout_delivery_retry_plan;
  if (item?.final_reporting_release_closeout_delivery_retry_worker_run && typeof item.final_reporting_release_closeout_delivery_retry_worker_run === "object") return item.final_reporting_release_closeout_delivery_retry_worker_run;
  if (item?.final_reporting_release_closeout_delivery_retry_execution_record && typeof item.final_reporting_release_closeout_delivery_retry_execution_record === "object") return item.final_reporting_release_closeout_delivery_retry_execution_record;
...
function goDrillDeliverySource(item) {
  ...
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-approval-decision") return "go_backend_rollback_drill_acknowledgement_audit";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-record-attachment") return "go_backend_rollback_drill_acknowledgement_audit";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closure-packet-verification") return "go_backend_rollback_drill_acknowledgement_audit";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export") return "go_backend_rollback_drill_acknowledgement_audit";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-plan") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-worker-run") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-execution-record") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-immutable-archive-reference") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export";
  ...
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health-alert-plan") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_closeout_retention_health_alert";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-plan") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-worker-run") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout";
  if (item?.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-execution-record") return "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout";
...
function buildSampleGoDrillNotificationDashboard(items) {
  ...
  const auditFinalReportingReadinessApprovals = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-approval-decision");
  const auditFinalReportingReleaseRecordAttachments = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-record-attachment");
  const auditFinalReportingClosurePacketVerifications = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closure-packet-verification");
  const auditFinalReportingAuditorExports = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export");
  const auditFinalReportingAuditorExportDeliveries = deliveries.filter((item) => item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export");
  const auditFinalReportingAuditorExportDeliveryRetryPlans = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-plan");
  const auditFinalReportingAuditorExportDeliveryRetryWorkerRuns = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-worker-run");
  const auditFinalReportingAuditorExportDeliveryRetryExecutions = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-execution-record");
  const auditFinalReportingArchiveReferences = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-immutable-archive-reference");
  const auditFinalReportingArchiveHealthReports = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-archive-reference-health");
  ...
  const auditFinalReportingCloseoutRetentionHealthReports = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health");
  const auditFinalReportingCloseoutRetentionHealthAlerts = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health-alert-plan");
  const auditFinalReportingCloseoutRetentionHealthAlertDeliveries = deliveries.filter((item) => item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_closeout_retention_health_alert");
  const auditFinalReportingCloseoutDeliveryRetryPlans = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-plan");
  const auditFinalReportingCloseoutDeliveryRetryWorkerRuns = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-worker-run");
  const auditFinalReportingCloseoutDeliveryRetryExecutions = items.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-execution-record");
  ...
  return {
    ...
    acknowledgement_audit_delivery_final_reporting_release_readiness_approval_decision_count: auditFinalReportingReadinessApprovals.length,
    acknowledgement_audit_delivery_final_reporting_release_readiness_approved_count: auditFinalReportingReadinessApprovals.filter((item) => item.approval_state === "approved").length,
    acknowledgement_audit_delivery_final_reporting_release_record_attachment_count: auditFinalReportingReleaseRecordAttachments.length,
    acknowledgement_audit_delivery_final_reporting_release_closure_packet_verification_count: auditFinalReportingClosurePacketVerifications.length,
    acknowledgement_audit_delivery_final_reporting_release_closure_packet_verified_count: auditFinalReportingClosurePacketVerifications.filter((item) => item.verification_state === "verified").length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_count: auditFinalReportingAuditorExports.length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_count: auditFinalReportingAuditorExportDeliveries.length,
    failed_acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_count: auditFinalReportingAuditorExportDeliveries.filter((item) => item.delivery_success === false).length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_plan_count: auditFinalReportingAuditorExportDeliveryRetryPlans.length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retryable_count: auditFinalReportingAuditorExportDeliveryRetryPlans.reduce((total, item) => total + Number(item.retryable_count || 0), 0),
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_worker_run_count: auditFinalReportingAuditorExportDeliveryRetryWorkerRuns.length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_execution_record_count: auditFinalReportingAuditorExportDeliveryRetryExecutions.length,
    acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_execution_failed_count: auditFinalReportingAuditorExportDeliveryRetryExecutions.filter((item) => ["failed", "skipped"].includes(item.execution_status)).length,
    acknowledgement_audit_delivery_final_reporting_immutable_archive_reference_count: auditFinalReportingArchiveReferences.length,
    acknowledgement_audit_delivery_final_reporting_archive_reference_health_count: auditFinalReportingArchiveHealthReports.length,
    acknowledgement_audit_delivery_final_reporting_archive_reference_health_alert_count: auditFinalReportingArchiveHealthReports.reduce((total, item) => total + Number(item.alert_count || 0), 0),
    acknowledgement_audit_delivery_final_reporting_archive_reference_health_alert_plan_count: auditFinalReportingArchiveHealthAlerts.length,
    ...
    acknowledgement_audit_delivery_final_reporting_closeout_retention_health_count: auditFinalReportingCloseoutRetentionHealthReports.length,
    acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_count: auditFinalReportingCloseoutRetentionHealthReports.reduce((total, item) => total + Number(item.alert_count || 0), 0),
    acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_plan_count: auditFinalReportingCloseoutRetentionHealthAlerts.length,
    acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_delivery_count: auditFinalReportingCloseoutRetentionHealthAlertDeliveries.length,
    failed_acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_delivery_count: auditFinalReportingCloseoutRetentionHealthAlertDeliveries.filter((item) => item.delivery_success === false).length,
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_plan_count: auditFinalReportingCloseoutDeliveryRetryPlans.length,
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retryable_count: auditFinalReportingCloseoutDeliveryRetryPlans.reduce((total, item) => total + Number(item.retryable_count || 0), 0),
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_worker_run_count: auditFinalReportingCloseoutDeliveryRetryWorkerRuns.length,
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_execution_record_count: auditFinalReportingCloseoutDeliveryRetryExecutions.length,
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_execution_success_count: auditFinalReportingCloseoutDeliveryRetryExecutions.filter((item) => item.execution_status === "delivered").length,
    acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_execution_failed_count: auditFinalReportingCloseoutDeliveryRetryExecutions.filter((item) => ["failed", "skipped"].includes(item.execution_status)).length,
...
function renderGoRollbackDrillNotifications(historyItems, dashboard = {}, routingRows = [], suppressionTrend = {}, retryRecoveryReport = {}) {
  ...
  panel.innerHTML = `
    ...
    <div class="release-delivery-metric">
      ...
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_release_readiness_approved_count || 0) ? "allow" : "require_approval"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_release_readiness_approval_decision_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-approval-decision").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Release Attachments</span>
      <strong>${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_release_record_attachment_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-record-attachment").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Packet Verified</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_release_closure_packet_verified_count || 0) ? "allow" : "require_approval"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_release_closure_packet_verification_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closure-packet-verification").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Auditor Exports</span>
      <strong>${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Auditor Deliveries</span>
      <strong class="${Number(dashboard.failed_acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_count || 0) ? "block" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_count || historyItems.filter((item) => item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Auditor Retryable</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retryable_count || 0) ? "require_approval" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retryable_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Auditor Retry Runs</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_execution_failed_count || 0) ? "block" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_auditor_export_delivery_retry_worker_run_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Archive Refs</span>
      <strong>${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_immutable_archive_reference_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-immutable-archive-reference").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Archive Alerts</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_archive_reference_health_alert_count || 0) ? "block" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_archive_reference_health_alert_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
    ...
    <div class="release-delivery-metric">
      ...
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_count || 0) ? "block" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_closeout_retention_health_count || historyItems.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health").length)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Retention Alerts</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_delivery_count || 0) ? "require_approval" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_closeout_retention_health_alert_plan_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Closeout Retryable</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retryable_count || 0) ? "require_approval" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retryable_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
      <span>Closeout Retry Runs</span>
      <strong class="${Number(dashboard.acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_execution_failed_count || 0) ? "block" : "allow"}">${formatMetricNumber(dashboard.acknowledgement_audit_delivery_final_reporting_release_closeout_delivery_retry_worker_run_count || 0)}</strong>
    </div>
    <div class="release-delivery-metric">
...
function goDrillAckExternalRef() {
  return document.querySelector("#goDrillAckExternalRef")?.value.trim() || "";
}
...
function latestGoDrillReleaseReadinessSummary() {
  const item = goRollbackDrillNotificationCatalog.find((record) => (
    record.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-summary"
  ));
  return item?.final_reporting_release_readiness_summary && typeof item.final_reporting_release_readiness_summary === "object"
    ? item.final_reporting_release_readiness_summary
    : item;
}

function latestGoDrillReleaseReadinessApproval() {
  const item = goRollbackDrillNotificationCatalog.find((record) => (
    record.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-readiness-approval-decision"
    && record.approval_state === "approved"
  ));
  return item?.final_reporting_release_readiness_approval_decision && typeof item.final_reporting_release_readiness_approval_decision === "object"
    ? item.final_reporting_release_readiness_approval_decision
    : item;
}

function latestGoDrillReleaseRecordAttachment() {
  const item = goRollbackDrillNotificationCatalog.find((record) => (
    record.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-record-attachment"
  ));
  return item?.final_reporting_release_record_attachment && typeof item.final_reporting_release_record_attachment === "object"
    ? item.final_reporting_release_record_attachment
    : item;
}

function latestGoDrillAuditorExport() {
  const item = goRollbackDrillNotificationCatalog.find((record) => (
    record.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export"
  ));
  return item?.final_reporting_auditor_export && typeof item.final_reporting_auditor_export === "object"
    ? item.final_reporting_auditor_export
    : item;
}

function latestGoDrillPayloadByKind(kind) {
  const item = goRollbackDrillNotificationCatalog.find((record) => record.metadata_kind === kind);
  return goDrillNotificationPayload(item);
}

async function approveGoDrillReleaseReadiness() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Approving final reporting release readiness...";
  try {
    let summary = latestGoDrillReleaseReadinessSummary();
    if (!summary?.summary_id) {
      await showGoDrillReleaseReadinessSummary();
      summary = latestGoDrillReleaseReadinessSummary();
    }
    if (!summary?.summary_id) throw new Error("release readiness summary unavailable");
    const response = await fetch(apiUrl(`/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-readiness/${encodeURIComponent(summary.summary_id)}/approval-decisions`), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        decided_by: goDrillAckActor(),
        ...
        external_ref: goDrillAckExternalRef() || "REL-local",
        notes: document.querySelector("#goDrillAckNotes")?.value || "Approved from Evidence Console"
      })
    });
    if (!response.ok) throw new Error(await response.text() || "release readiness approval API unavailable");
    const result = await response.json();
    const decision = result.decision || {};
    if (panel) {
      panel.innerHTML = `
        <dl>
          <dt>Decision</dt><dd>${escapeHtml(decision.decision_id || "unknown")}</dd>
          <dt>State</dt><dd class="${decision.approval_state === "approved" ? "allow" : "require_approval"}">${escapeHtml(decision.approval_state || "unknown")}</dd>
          <dt>Summary</dt><dd>${escapeHtml(decision.summary_id || "")}</dd>
        </dl>
        <pre>${escapeHtml(JSON.stringify(decision, null, 2))}</pre>
      `;
    }
    if (status) status.textContent = `Release readiness approval ${decision.decision_id || ""} recorded.`;
  } catch (error) {
    const summary = latestGoDrillReleaseReadinessSummary() || { summary_id: `sample-go-drill-final-readiness-${Date.now()}`, readiness_state: "blocked", closure_state: "open" };
    const decidedAt = new Date().toISOString();
    const decisionId = `sample-go-drill-final-approval-${Date.now()}`;
    const decision = {
      decision_id: decisionId,
      summary_id: summary.summary_id,
      readiness_state: summary.readiness_state || "blocked",
      closure_state: summary.closure_state || "open",
      approval_state: "approved",
      override_blockers: true,
      decided_by: goDrillAckActor(),
      decided_at: decidedAt,
    ...
    if (status) status.textContent = `Using local sample release readiness approval: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function attachGoDrillReleaseRecord() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Attaching final reporting evidence to release record...";
  try {
    let approval = latestGoDrillReleaseReadinessApproval();
    if (!approval?.decision_id) {
      await approveGoDrillReleaseReadiness();
      approval = latestGoDrillReleaseReadinessApproval();
    }
    if (!approval?.decision_id) throw new Error("approved release readiness decision unavailable");
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-record-attachment"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        attached_by: goDrillAckActor(),
        approval_decision_id: approval.decision_id,
        release_record_ref: goDrillAckExternalRef() || "REL-local",
        notes: document.querySelector("#goDrillAckNotes")?.value || "Attached from Evidence Console"
      })
    });
    if (!response.ok) throw new Error(await response.text() || "release record attachment API unavailable");
    const result = await response.json();
    const attachment = result.attachment || {};
    if (panel) {
      panel.innerHTML = `
        <dl>
          <dt>Attachment</dt><dd>${escapeHtml(attachment.attachment_id || "unknown")}</dd>
          <dt>Release Record</dt><dd>${escapeHtml(attachment.release_record_ref || "")}</dd>
          <dt>State</dt><dd class="allow">${escapeHtml(attachment.attachment_state || "unknown")}</dd>
        </dl>
        <pre>${escapeHtml(JSON.stringify(attachment, null, 2))}</pre>
      `;
    }
    if (status) status.textContent = `Release record attachment ${attachment.attachment_id || ""} captured.`;
  } catch (error) {
    const approval = latestGoDrillReleaseReadinessApproval() || { decision_id: `sample-go-drill-final-approval-${Date.now()}`, summary_id: "sample" };
    const attachedAt = new Date().toISOString();
    const attachmentId = `sample-go-drill-final-attachment-${Date.now()}`;
    const attachment = {
    ...
    goRollbackDrillNotificationCatalog.unshift({
      ...
      signer: attachment.attached_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-record-attachment",
      attachment_id: attachmentId,
      release_record_ref: attachment.release_record_ref,
      attachment_state: "attached",
      approval_decision_id: attachment.approval_decision_id,
      final_reporting_release_record_attachment: attachment
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(attachment, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample release record attachment: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function verifyGoDrillClosurePacket() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Verifying final reporting release closure packet...";
  try {
    let attachment = latestGoDrillReleaseRecordAttachment();
    if (!attachment?.attachment_id) {
      await attachGoDrillReleaseRecord();
      attachment = latestGoDrillReleaseRecordAttachment();
    }
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-closure-packet-verification", {
      release_record_ref: attachment?.release_record_ref || goDrillAckExternalRef() || "REL-local",
      generated_by: goDrillAckActor(),
      persist: true
    }));
    if (!response.ok) throw new Error(await response.text() || "closure packet verification API unavailable");
    const result = await response.json();
    const verification = result.verification || {};
    if (panel) {
      panel.innerHTML = `
        <dl>
          <dt>Verification</dt><dd>${escapeHtml(verification.verification_id || "unknown")}</dd>
          <dt>State</dt><dd class="${verification.verification_state === "verified" ? "allow" : "require_approval"}">${escapeHtml(verification.verification_state || "unknown")}</dd>
          <dt>Release Record</dt><dd>${escapeHtml(verification.release_record_ref || "")}</dd>
        </dl>
        <pre>${escapeHtml(JSON.stringify(verification, null, 2))}</pre>
      `;
    }
    if (status) status.textContent = `Closure packet verification ${verification.verification_state || "unknown"}.`;
  } catch (error) {
    const attachment = latestGoDrillReleaseRecordAttachment() || { attachment_id: `sample-go-drill-final-attachment-${Date.now()}`, release_record_ref: goDrillAckExternalRef() || "REL-local" };
    const generatedAt = new Date().toISOString();
    const verificationId = `sample-go-drill-final-packet-${Date.now()}`;
    const verification = {
    ...
    goRollbackDrillNotificationCatalog.unshift({
      ...
      created_at: generatedAt,
      signer: verification.generated_by,
      ...
      verification_id: verificationId,
      verification_state: "verified",
      release_record_ref: verification.release_record_ref,
      final_reporting_release_closure_packet_verification: verification
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(verification, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample closure packet verification: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function exportGoDrillAuditorPacket() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Exporting final reporting auditor packet...";
  try {
    const attachment = latestGoDrillReleaseRecordAttachment();
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-auditor-export"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "auditor export API unavailable");
    const result = await response.json();
    const exported = result.export || {};
    if (panel) {
      panel.innerHTML = `
        <dl>
          <dt>Export</dt><dd>${escapeHtml(exported.export_id || "unknown")}</dd>
          <dt>Verification</dt><dd class="${exported.verification_state === "verified" ? "allow" : "require_approval"}">${escapeHtml(exported.verification_state || "unknown")}</dd>
          <dt>Release Record</dt><dd>${escapeHtml(exported.release_record_ref || "")}</dd>
        </dl>
        <pre>${escapeHtml(exported.markdown || JSON.stringify(exported, null, 2))}</pre>
      `;
    }
    if (status) status.textContent = `Auditor export ${exported.export_id || ""} captured.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const exportId = `sample-go-drill-final-auditor-${Date.now()}`;
    const exported = {
      export_id: exportId,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      verification_state: "verified",
      release_record_ref: goDrillAckExternalRef() || "REL-local",
      markdown: "# CAVRA Rollback Drill Final Reporting Auditor Export\n\n- Source: local sample\n- Verification state: verified\n"
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: exportId,
      created_at: generatedAt,
      signer: exported.generated_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export",
      export_id: exportId,
      verification_state: "verified",
      release_record_ref: exported.release_record_ref,
      final_reporting_auditor_export: exported
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(exported.markdown)}</pre>`;
    if (status) status.textContent = `Using local sample auditor export: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function ensureGoDrillAuditorExport() {
  let exported = latestGoDrillAuditorExport();
  if (!exported?.export_id) {
    await exportGoDrillAuditorPacket();
    exported = latestGoDrillAuditorExport();
  }
  return exported;
}

async function deliverGoDrillAuditorPacket() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Delivering final reporting auditor packet...";
  const provider = document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook";
  try {
    const exported = await ensureGoDrillAuditorExport();
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-auditor-export/deliver"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
        release_record_ref: exported?.release_record_ref || goDrillAckExternalRef() || "REL-local",
        provider,
        retries: 0,
        timeout_seconds: 0.1
      })
    });
    if (!response.ok) throw new Error(await response.text() || "auditor export delivery API unavailable");
    const result = await response.json();
    const metadata = result.metadata || {};
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(metadata, null, 2))}</pre>`;
    if (status) status.textContent = `Auditor export delivered through ${provider}.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const exported = latestGoDrillAuditorExport() || {};
    const deliveryId = `sample-go-drill-final-auditor-delivery-${Date.now()}`;
    const metadata = {
      session_id: deliveryId,
      created_at: generatedAt,
      signer: goDrillAckActor(),
      ...
      event_id: exported.export_id || deliveryId,
      export_id: exported.export_id || deliveryId,
      verification_id: exported.verification_id || "",
      release_record_ref: exported.release_record_ref || goDrillAckExternalRef() || "REL-local",
      delivery_success: true,
      providers: [provider],
      failed_providers: [],
    ...
    goRollbackDrillNotificationCatalog.unshift(metadata);
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(metadata, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample auditor delivery: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function planGoDrillAuditorDeliveryRetry() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Planning final auditor export delivery retries...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-auditor-export/delivery-retry-plan"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "auditor retry plan API unavailable");
    const result = await response.json();
    const plan = result.plan || {};
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(plan, null, 2))}</pre>`;
    if (status) status.textContent = `Auditor retry plan has ${formatMetricNumber(plan.retryable_count || 0)} retryable deliveries.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const planId = `sample-go-drill-final-auditor-retry-${Date.now()}`;
    const failed = goRollbackDrillNotificationCatalog.filter((item) => (
      item.metadata_kind === "release-connector-delivery"
      && item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_auditor_export"
      && item.delivery_success === false
    ));
    const plan = {
      retry_plan_id: planId,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      alert_level: failed.length ? "critical" : "healthy",
      decision_count: failed.length,
      retryable_count: failed.length,
      waiting_count: 0,
      suppressed_count: 0,
      retry_decisions: failed.map((item) => ({
        export_id: item.export_id || item.event_id || "",
        provider: (item.failed_providers || item.providers || ["webhook"])[0],
        action: "retry",
        reason: "local sample failed final auditor delivery is eligible for retry",
        latest_delivery_id: item.session_id || ""
      }))
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: planId,
      ...
      retry_plan_id: planId,
      alert_level: plan.alert_level,
      retryable_count: plan.retryable_count,
      final_reporting_auditor_export_delivery_retry_plan: plan
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(plan, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample auditor retry plan: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function archiveGoDrillAuditorPacket() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Recording final reporting archive reference...";
  try {
    const exported = await ensureGoDrillAuditorExport();
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-immutable-archive-reference"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        archived_by: goDrillAckActor(),
        release_record_ref: exported?.release_record_ref || goDrillAckExternalRef() || "REL-local",
        archive_ref: `immutable-archive://local/${encodeURIComponent(exported?.export_id || Date.now())}`,
        archive_provider: "operator_archive",
        legal_hold: false,
        notes: document.querySelector("#goDrillAckNotes")?.value || "Archive reference captured from Evidence Console"
      })
    });
    if (!response.ok) throw new Error(await response.text() || "archive reference API unavailable");
    const result = await response.json();
    const reference = result.reference || {};
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(reference, null, 2))}</pre>`;
    if (status) status.textContent = `Archive reference ${reference.reference_id || ""} captured.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const exported = latestGoDrillAuditorExport() || {};
    const referenceId = `sample-go-drill-final-archive-${Date.now()}`;
    const reference = {
      reference_id: referenceId,
      created_at: generatedAt,
      archived_at: generatedAt,
      archived_by: goDrillAckActor(),
      archive_ref: `immutable-archive://local/${encodeURIComponent(exported.export_id || referenceId)}`,
      archive_provider: "operator_archive",
      legal_hold: false,
      export_id: exported.export_id || "",
      verification_state: exported.verification_state || "verified",
      release_record_ref: exported.release_record_ref || goDrillAckExternalRef() || "REL-local"
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: referenceId,
      created_at: generatedAt,
      signer: reference.archived_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-immutable-archive-reference",
      reference_id: referenceId,
      archive_provider: reference.archive_provider,
      archive_ref: reference.archive_ref,
      export_id: reference.export_id,
      verification_state: reference.verification_state,
      release_record_ref: reference.release_record_ref,
      final_reporting_immutable_archive_reference: reference
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(reference, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample archive reference: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function checkGoDrillArchiveHealth() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Checking final archive reference health...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-archive-reference-health", {
      generated_by: goDrillAckActor()
    }));
    if (!response.ok) throw new Error(await response.text() || "archive health API unavailable");
    const result = await response.json();
    const health = result.health || {};
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(health, null, 2))}</pre>`;
    if (status) status.textContent = `Archive health is ${health.alert_level || "unknown"} with ${formatMetricNumber(health.alert_count || 0)} alerts.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const exports = goRollbackDrillNotificationCatalog.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export");
    const archived = new Set(goRollbackDrillNotificationCatalog.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-immutable-archive-reference").map((item) => item.export_id));
    const alerts = exports.filter((item) => !archived.has(item.export_id)).map((item) => ({
      alert_id: "archive_reference_missing",
      severity: "critical",
      export_id: item.export_id,
      message: "verified auditor export has no immutable archive reference"
    }));
    const healthId = `sample-go-drill-final-archive-health-${Date.now()}`;
    const health = {
      health_id: healthId,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      alert_level: alerts.length ? "critical" : "healthy",
      checked_export_count: exports.length,
      archived_export_count: exports.length - alerts.length,
    ...
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(health, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample archive health: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function runGoDrillAuditorDeliveryRetry() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  const provider = document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook";
  if (status) status.textContent = "Running final auditor export retry worker...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-auditor-export/delivery-retry-worker-run"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
        execute: true,
        provider,
        retry_policy: { allow_immediate_retry: true, retry_delay_minutes: 0 },
        retries: 0,
        timeout_seconds: 0.1
      })
    });
    if (!response.ok) throw new Error(await response.text() || "auditor retry worker API unavailable");
    const result = await response.json();
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(result.run || {}, null, 2))}</pre>`;
    if (status) status.textContent = `Auditor retry worker selected ${formatMetricNumber(result.run?.summary?.selected_retry_count || 0)} deliveries.`;
  } catch (error) {
    const plan = latestGoDrillPayloadByKind("go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-auditor-export-delivery-retry-plan") || {};
    const generatedAt = new Date().toISOString();
    const runId = `sample-go-drill-final-auditor-retry-worker-${Date.now()}`;
    const selected = Array.isArray(plan.retry_decisions) ? plan.retry_decisions.filter((item) => item.action === "retry") : [];
    const run = {
      run_id: runId,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      dry_run: false,
      summary: { selected_retry_count: selected.length, retryable_count: selected.length },
      retry_plan: plan,
      selected_retries: selected
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: runId,
      created_at: generatedAt,
      signer: run.generated_by,
      ...
      run_id: runId,
      dry_run: false,
      selected_retry_count: selected.length,
      final_reporting_auditor_export_delivery_retry_worker_run: run
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(run, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample auditor retry worker: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function sendGoDrillArchiveHealthAlert() {
  const status = document.querySelector("#goDrillAckStatus");
  const provider = document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook";
  if (status) status.textContent = "Sending final archive health alert...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-archive-reference-health-alerts/deliver"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "archive health alert API unavailable");
    const result = await response.json();
    if (status) status.textContent = `Archive health alert ${result.plan?.plan_id || ""} selected ${formatMetricNumber(result.plan?.selected_providers?.length || 0)} destinations.`;
  } catch (error) {
    const health = latestGoDrillPayloadByKind("go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-archive-reference-health") || {};
    const generatedAt = new Date().toISOString();
    const planId = `sample-go-drill-final-archive-alert-${Date.now()}`;
    goRollbackDrillNotificationCatalog.unshift({
      session_id: planId,
      created_at: generatedAt,
      signer: goDrillAckActor(),
      ...
      alert_level: health.alert_level || "warning",
      selected_providers: [provider],
      acknowledgement_required_providers: [provider],
    ...
    if (status) status.textContent = `Using local sample archive alert: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function ackGoDrillArchiveHealthAlert() {
  const status = document.querySelector("#goDrillAckStatus");
  const provider = document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook";
  const plan = latestGoDrillPayloadByKind("go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-archive-reference-health-alert-plan") || {};
  const healthId = plan.health_id || "";
  if (status) status.textContent = "Acknowledging final archive health alert...";
  try {
    const response = await fetch(apiUrl(`/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-archive-reference-health-alerts/${encodeURIComponent(healthId)}/acknowledgements`), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        acknowledged_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "archive health alert acknowledgement API unavailable");
    const result = await response.json();
    if (status) status.textContent = `Archive health alert acknowledged: ${result.acknowledgement?.acknowledgement_id || ""}.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const acknowledgementId = `sample-go-drill-final-archive-alert-ack-${Date.now()}`;
    goRollbackDrillNotificationCatalog.unshift({
      session_id: acknowledgementId,
      created_at: generatedAt,
      signer: goDrillAckActor(),
      ...
      provider,
      acknowledgement_state: "acknowledged",
      acknowledgement: { acknowledgement_id: acknowledgementId, health_id: healthId, plan_id: plan.plan_id || "", provider, acknowledgement_state: "acknowledged" }
    });
    if (status) status.textContent = `Using local sample archive alert acknowledgement: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}
...
async function checkGoDrillCloseoutRetentionHealth() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Checking closeout retention health...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-closeout-retention-health", {
      generated_by: goDrillAckActor(),
      expiry_warning_days: 30
    }));
    if (!response.ok) throw new Error(await response.text() || "retention health API unavailable");
    const result = await response.json();
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(result.health || result, null, 2))}</pre>`;
    if (status) status.textContent = `Retention health is ${result.health?.alert_level || "unknown"} with ${formatMetricNumber(result.health?.alert_count || 0)} alerts.`;
  } catch (error) {
    const generatedAt = new Date().toISOString();
    const health = {
      health_id: `sample-go-drill-final-retention-health-${Date.now()}`,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      alert_level: "warning",
      artifact_bundle_count: goRollbackDrillNotificationCatalog.filter((item) => item.metadata_kind === "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-artifact-bundle").length,
      failed_closeout_delivery_count: goRollbackDrillNotificationCatalog.filter((item) => item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout" && item.delivery_success === false).length,
      alert_count: 1,
      findings: [{ severity: "warning", category: "local_sample", message: error.message || "API unavailable" }]
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: health.health_id,
      created_at: generatedAt,
      signer: health.generated_by,
      ...
      alert_level: health.alert_level,
      alert_count: health.alert_count,
      failed_closeout_delivery_count: health.failed_closeout_delivery_count,
      final_reporting_closeout_retention_health: health
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(health, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample retention health: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function sendGoDrillCloseoutRetentionHealthAlert() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Sending closeout retention health alert...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-closeout-retention-health-alerts/deliver"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "retention alert API unavailable");
    const result = await response.json();
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(result.plan || result, null, 2))}</pre>`;
    if (status) status.textContent = `Retention health alert ${result.plan?.plan_id || ""} prepared.`;
  } catch (error) {
    const health = latestGoDrillPayloadByKind("go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health") || {};
    const generatedAt = new Date().toISOString();
    const plan = {
      plan_id: `sample-go-drill-final-retention-alert-${Date.now()}`,
      health_id: health.health_id || "sample-health",
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      alert_level: health.alert_level || "warning",
      selected_providers: [document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook"],
      acknowledgement_required_providers: [document.querySelector("#goDrillAckAuditDeliveryProvider")?.value || "webhook"]
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: plan.plan_id,
      created_at: generatedAt,
      signer: plan.generated_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-closeout-retention-health-alert-plan",
      plan_id: plan.plan_id,
      health_id: plan.health_id,
      alert_level: plan.alert_level,
      selected_providers: plan.selected_providers,
      final_reporting_closeout_retention_health_alert_plan: plan
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(plan, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample retention alert: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function planGoDrillCloseoutDeliveryRetry() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Planning closeout delivery retry...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-closeout-summary/delivery-retry-plan"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "closeout retry plan API unavailable");
    const result = await response.json();
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(result.plan || result, null, 2))}</pre>`;
    if (status) status.textContent = `Closeout retry plan has ${formatMetricNumber(result.plan?.retryable_count || 0)} retryable deliveries.`;
  } catch (error) {
    const failed = goRollbackDrillNotificationCatalog.filter((item) => item.connector_delivery_source === "go_backend_rollback_drill_acknowledgement_audit_final_reporting_release_closeout" && item.delivery_success === false);
    const generatedAt = new Date().toISOString();
    const plan = {
      retry_plan_id: `sample-go-drill-final-closeout-retry-${Date.now()}`,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      retryable_count: failed.length,
      retry_decisions: failed.map((item) => ({ delivery_id: item.session_id, summary_id: item.summary_id || item.event_id, provider: item.provider || "webhook", action: "retry" }))
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: plan.retry_plan_id,
      created_at: generatedAt,
      signer: plan.generated_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-plan",
      retry_plan_id: plan.retry_plan_id,
      retryable_count: plan.retryable_count,
      final_reporting_release_closeout_delivery_retry_plan: plan
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(plan, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample closeout retry plan: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}

async function runGoDrillCloseoutDeliveryRetry() {
  const status = document.querySelector("#goDrillAckStatus");
  const panel = document.querySelector("#goDrillNotificationDetail");
  if (status) status.textContent = "Running closeout delivery retry worker...";
  try {
    const response = await fetch(apiUrl("/runtime/go-pilot/rollback-drill-notifications/acknowledgements/audit-delivery/final-reporting-release-closeout-summary/delivery-retry-worker-run"), {
      method: "POST",
      headers: apiHeaders(true),
      body: JSON.stringify({
        generated_by: goDrillAckActor(),
    ...
    if (!response.ok) throw new Error(await response.text() || "closeout retry worker API unavailable");
    const result = await response.json();
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(result.run || result, null, 2))}</pre>`;
    if (status) status.textContent = `Closeout retry worker selected ${formatMetricNumber(result.run?.summary?.selected_retry_count || 0)} deliveries.`;
  } catch (error) {
    const plan = latestGoDrillPayloadByKind("go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-plan") || {};
    const generatedAt = new Date().toISOString();
    const run = {
      run_id: `sample-go-drill-final-closeout-retry-worker-${Date.now()}`,
      generated_at: generatedAt,
      generated_by: goDrillAckActor(),
      dry_run: true,
      retry_plan_id: plan.retry_plan_id || "",
      selected_retries: Array.isArray(plan.retry_decisions) ? plan.retry_decisions.filter((item) => item.action === "retry") : [],
      summary: { selected_retry_count: Array.isArray(plan.retry_decisions) ? plan.retry_decisions.filter((item) => item.action === "retry").length : 0 }
    };
    goRollbackDrillNotificationCatalog.unshift({
      session_id: run.run_id,
      created_at: generatedAt,
      signer: run.generated_by,
      metadata_kind: "go-backend-rollback-drill-acknowledgement-audit-delivery-final-reporting-release-closeout-delivery-retry-worker-run",
      run_id: run.run_id,
      retry_plan_id: run.retry_plan_id,
      dry_run: true,
      selected_retry_count: run.summary.selected_retry_count,
      final_reporting_release_closeout_delivery_retry_worker_run: run
    });
    if (panel) panel.innerHTML = `<pre>${escapeHtml(JSON.stringify(run, null, 2))}</pre>`;
    if (status) status.textContent = `Using local sample closeout retry worker: ${error.message || "API unavailable"}.`;
  }
  await refreshGoRollbackDrillNotifications();
}
...
document.querySelector("#goDrillApproveReleaseReadiness").addEventListener("click", approveGoDrillReleaseReadiness);
document.querySelector("#goDrillAttachReleaseRecord").addEventListener("click", attachGoDrillReleaseRecord);
document.querySelector("#goDrillVerifyClosurePacket").addEventListener("click", verifyGoDrillClosurePacket);
document.querySelector("#goDrillExportAuditorPacket").addEventListener("click", exportGoDrillAuditorPacket);
document.querySelector("#goDrillDeliverAuditorPacket").addEventListener("click", deliverGoDrillAuditorPacket);
document.querySelector("#goDrillPlanAuditorDeliveryRetry").addEventListener("click", planGoDrillAuditorDeliveryRetry);
document.querySelector("#goDrillRunAuditorDeliveryRetry").addEventListener("click", runGoDrillAuditorDeliveryRetry);
document.querySelector("#goDrillArchiveAuditorPacket").addEventListener("click", archiveGoDrillAuditorPacket);
...
document.querySelector("#goDrillCheckCloseoutRetentionHealth").addEventListener("click", checkGoDrillCloseoutRetentionHealth);
document.querySelector("#goDrillSendCloseoutRetentionHealthAlert").addEventListener("click", sendGoDrillCloseoutRetentionHealthAlert);
document.querySelector("#goDrillPlanCloseoutDeliveryRetry").addEventListener("click", planGoDrillCloseoutDeliveryRetry);
document.querySelector("#goDrillRunCloseoutDeliveryRetry").addEventListener("click", runGoDrillCloseoutDeliveryRetry);
